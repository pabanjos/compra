<section>
	<form class="pure-form">
		<table data-ng-init="exibir('clientes')">
			<thead class="colorAdministrador">
				<tr>
					<th colspan="3">Conta</th>
					<th colspan="6">Pessoa</th>
				</tr>
				<tr class="coluna">
					<th data-ng-click="order='usuario';direction=!direction">Usuario <i class="fa fa-sort"></i></th>
				...
				<tr>
					<th><input type="search" data-ng-model="search.usuario" placeholder="usuario.." class="pure-input-rounded pure-input-1"></th>
			...
			<tbody data-ng-init="limit=8">
				<tr data-ng-repeat="c in lista | filter:search | orderBy:order:direction | limitTo:limit" data-ng-click="selecionar(c)" class="selecao">
					<th>{{c.usuario}}</th>
					<th>{{c.senha}}</th>
					<th>{{c.saldo | currency : "R$" }}</th>
					<th>{{c.pessoa.nome}}</th>
					<th>{{c.pessoa.email}}</th>
					<th>{{c.pessoa.sexo}}</th>
					<th>{{c.pessoa.telefone}}</th>
					<th>{{c.pessoa.cep}}</th>
					<th>{{c.pessoa.nascimento | date:'dd/MM/yyyy'}}</th>
				</tr>
			</tbody>
	...
	<form class="pure-form" data-ng-show="selecionado">
		<table>
			...
			<tbody>
				<tr>
					<th><input type="text" name="usuario" data-ng-model="selecionado.usuario" data-ng-value="selecionado.usuario" maxlength="15" class="pure-input-1"></th>
			...
			<tfoot class="colorAdministrador">
				<tr>
					<th colspan="3">
						<button data-ng-click="tornar(selecionado.idConta)" class="pure-button pure-input-1 button-warning">
							<i class="fa fa-key"></i> Tornar Administrador
						</button>
					</th>
					<th colspan="3">
						<button data-ng-click="atualizar(selecionado.idConta)" class="pure-button pure-input-1 pure-button-primary">
							<i class="fa fa-refresh"></i> Atualizar
						</button>
					</th>
					<th colspan="3">
						<button data-ng-click="deletar(selecionado.idConta)" class="pure-button pure-input-1 button-error">
